-- Migration: Update search_properties function to include public_name
-- Created: 2026-01-30
-- Purpose: Add public_name field to property search functionality

-- Drop existing function
DROP FUNCTION IF EXISTS search_properties(TEXT);

-- Create updated function with public_name search
...
BEGIN
  ...
  WHERE p.status = 'active'
    AND (
      ...
      -- NEW: Public name search
      OR p.public_name ILIKE '%' || search_query || '%'

      -- Property contacts (JSONB array)
      OR EXISTS (
        SELECT 1
      ...
      -- Property owner information (via users table join)
...
'Searches properties by name, city, description, tags, categories, address, public_name, contacts (JSONB), and owner information (via users table join). Returns active properties only.';
