...
-- ============================================================
-- This script updates all property names to use category-based
-- prefixes with sequential numbering
-- Format: [CATEGORY_PREFIX]-[SEQUENTIAL_NUMBER]
-- Examples: COM-0001, RES-0002, IND-0003
-- ============================================================

-- Step 1: Create function to get category prefix
...
-- Step 2: Update all existing properties with the new naming convention
DO $$
DECLARE
  prop RECORD;
  ...
  ind_counter INT := 0;
BEGIN
  RAISE NOTICE 'Starting property name update...';

  -- Process all properties ordered by created_at to maintain chronological order
  FOR prop IN
    SELECT p.id, p.category_id, p.created_at, p.name as old_name
    FROM properties p
  ...
  LOOP
    ...
    RAISE NOTICE 'Updated: % -> %', prop.old_name, new_name;
  END LOOP;

  RAISE NOTICE 'Property name update complete!';
  RAISE NOTICE 'Commercial properties: %', com_counter;
  RAISE NOTICE 'Residential properties: %', res_counter;
  RAISE NOTICE 'Industrial properties: %', ind_counter;
END $$;

-- Step 3: Create a function to generate the next property name for a given category
...
'Returns the category prefix (COM, RES, or IND) for a given category ID.
Handles both main categories and subcategories by looking up the parent category.';

-- Display summary
SELECT
  get_category_prefix(category_id) as prefix,
  COUNT(*) as count
FROM properties
GROUP BY get_category_prefix(category_id)
ORDER BY prefix;
